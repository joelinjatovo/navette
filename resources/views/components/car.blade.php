@if($model && $model->getKey())
	<div class="d-flex align-items-center">
		<div class="symbol symbol-40 symbol-light-success flex-shrink-0">
			...
			@else
				<span class="symbol-label font-size-h4 font-weight-bold">V</span>
			@endif
		</div>
		<div class="ml-4">
			...
			<a href="{{ auth()->check() && auth()->user()->isAdmin() ? route('admin.car.show', $model) : '#' }}" class="text-muted font-weight-bold text-hover-primary">{{ $model->type }}</a>
		</div>
	</div>
@else
	<div class="d-flex align-items-center">
